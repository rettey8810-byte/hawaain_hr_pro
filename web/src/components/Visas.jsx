...
export default function Visas() {
  ...
  useEffect(() => {
    ...
    if (activeFilter) {
      filtered = filtered.filter(v => {
        const days = calculateDaysRemaining(v.expiryDate);
        switch (activeFilter) {
          case 'expired':
            return days <= 0;
          case '30days':
  ...
  }, [visas, employeeId, searchTerm, employees, activeFilter]);
  ...
  const exportFilteredToCSV = () => {
    const headers = ['Employee Name', 'Employee ID', 'Passport Number', 'Visa Number', 'Visa Type', 'Entry Type', 'Expiry Date', 'Status', 'Days Remaining'];
    
    const rows = filteredVisas.map(v => {
      const empInfo = getEmployeeInfo(v.employeeId);
      const days = calculateDaysRemaining(v.expiryDate);
      return [
        empInfo.name,
        empInfo.empId,
    ...
    a.download = `visas${filterLabel}_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        ...
        >
          <div className="flex items-center">
            ...
            <div><p className="text-xs text-white/80 font-medium">Expired</p><p className="text-2xl font-bold">{visas.filter(v => calculateDaysRemaining(v.expiryDate) <= 0).length}</p></div>
          </div>
        </button>
        <button 
        ...
        >
          <div className="flex items-center">
            <div className="p-2 bg-white/20 rounded-lg mr-3"><AlertTriangle className="h-5 w-5 text-white" /></div>
            <div><p className="text-xs text-white/80 font-medium">&lt; 30 days</p><p className="text-2xl font-bold">{visas.filter(v => {const days = calculateDaysRemaining(v.expiryDate); return days > 0 && days <= 30;}).length}</p></div>
          </div>
        </button>
        <button 
        ...
        >
          <div className="flex items-center">
            <div className="p-2 bg-white/20 rounded-lg mr-3"><AlertTriangle className="h-5 w-5 text-white" /></div>
            <div><p className="text-xs text-white/80 font-medium">30-60 days</p><p className="text-2xl font-bold">{visas.filter(v => {const days = calculateDaysRemaining(v.expiryDate); return days > 30 && days <= 60;}).length}</p></div>
          </div>
        </button>
        <button 
        ...
        >
          <div className="flex items-center">
            <div className="p-2 bg-white/20 rounded-lg mr-3"><AlertTriangle className="h-5 w-5 text-white" /></div>
            <div><p className="text-xs text-white/80 font-medium">60-90 days</p><p className="text-2xl font-bold">{visas.filter(v => {const days = calculateDaysRemaining(v.expiryDate); return days > 60 && days <= 90;}).length}</p></div>
          </div>
        </button>
      </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-blue-800">
              Filter: {activeFilter === 'expired' ? 'Expired Visas' : activeFilter === '30days' ? 'Expiring in < 30 days' : activeFilter === '60days' ? 'Expiring in 30-60 days' : 'Expiring in 60-90 days'}
            </span>
            <span className="text-sm text-blue-600">({filteredVisas.length} results)</span>
          </div>
          <div className="flex items-center gap-2">
            <button
