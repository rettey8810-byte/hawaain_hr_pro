...
export default function WorkPermits() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const records = employees
        .filter(emp => emp.WPNo || emp.WPExpiry)
        .map(emp => ({
          id: emp.id,
          employeeId: emp.id,
          permitNumber: emp.WPNo || 'N/A',
  ...
  }, [permitRecords, searchTerm, activeFilter]);

  const exportToCSV = () => {
    const headers = ['Employee ID', 'Name', 'Department', 'Permit Number', 'Expiry Date', 'Status', 'Days Remaining'];
    const rows = filteredRecords.map(p => {
      const days = calculateDaysRemaining(p.expiryDate);
      return [p.empId, p.employeeName, p.department, p.permitNumber, formatDate(p.expiryDate), days <= 0 ? 'Expired' : days <= 30 ? 'Expiring Soon' : 'Valid', days];
    });
    ...
    a.href = url;
    a.download = `work-permits_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="md:flex md:items-center md:justify-between bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 rounded-2xl p-6 text-white shadow-xl">
        <div>
          <h2 className="text-3xl font-bold">📋 Work Permits</h2>
          <p className="mt-1 text-sm text-white/80">Track employee work permits and expiry dates</p>
        </div>
        <div className="mt-4 flex md:ml-4 md:mt-0 space-x-2">
          {isHR?.() && (
            <>
              <button onClick={exportToCSV} className="inline-flex items-center rounded-xl bg-white px-5 py-2.5 text-sm font-bold text-indigo-600 shadow-lg hover:bg-gray-50">
                <Download className="h-5 w-5 mr-2" /> Export CSV
              </button>
              <Link to="/employees/new" className="inline-flex items-center rounded-xl bg-white px-5 py-2.5 text-sm font-bold text-indigo-600 shadow-lg hover:bg-gray-50">
        ...
        <button onClick={() => setActiveFilter(null)} 
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{permitRecords.length}</p>
            </div>
          </div>
        </button>
        {[
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-lg p-5 border border-white/50">
        <div className="relative">
          ...
          <input type="text" placeholder="Search by permit number or employee name..." value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="block w-full rounded-xl border-0 bg-gray-50 pl-12 pr-4 py-3 text-gray-900 shadow-sm ring-1 ring-gray-200 focus:ring-2 focus:ring-indigo-500" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Permit Number</th>
              ...
              return (
                <tr key={p.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    ...
                    <div className="text-sm text-gray-500">{p.empId}</div>
                  </td>
                  <td className="px-6 py-4 font-mono text-sm">{p.permitNumber}</td>
                  ...
                  <td className="px-6 py-4">
                    <Link to={`/employees/${p.employeeId}`} className="text-indigo-600 hover:text-indigo-900">
        ...
        {filteredRecords.length === 0 && (
          <div className="text-center py-12 text-gray-500">No work permit records found</div>
        )}
      </div>
    </div>
  );
